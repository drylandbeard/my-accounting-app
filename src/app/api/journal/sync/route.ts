...
export async function POST(req: NextRequest) {
  try {
    ...
    for (const tx of transactions) {
      ...
      const hasSplits = tx.split_data && tx.split_data.splits && tx.split_data.splits.length > 0;
      
      if (hasSplits) {
        // For split transactions:
        // 1. Single debit entry to the corresponding account (the account the money came from/went to)
        // 2. Multiple credit entries for each split item
        
        const totalAmount = tx.spent && tx.spent > 0 ? tx.spent : tx.received;
        const isSpent = tx.spent && tx.spent > 0;
        
        // Create the main debit entry (account being debited - where money comes from)
        journalEntries.push({
          id: uuidv4(),
          transaction_id: tx.id,
          date: tx.date,
          description: tx.description,
          chart_account_id: tx.corresponding_category_id, // The account (e.g., checking account)
          debit: isSpent ? totalAmount : 0,
          credit: isSpent ? 0 : totalAmount,
          company_id: companyId,
          split_data: tx.split_data, // Store split data for reference
        });
        
        // Create credit entries for each split
        for (const split of tx.split_data.splits) {
          const splitAmount = parseFloat(split.spent || split.received || '0');
          if (splitAmount > 0 && split.selected_category_id) {
            journalEntries.push({
              id: uuidv4(),
              transaction_id: tx.id,
              date: tx.date,
              description: split.description || tx.description,
              chart_account_id: split.selected_category_id, // The category being credited
              debit: isSpent ? 0 : splitAmount,
              credit: isSpent ? splitAmount : 0,
              company_id: companyId,
              split_data: null, // Individual split entries don't need the full split data
            });
          }
        }
      } else {
        // Regular transaction logic (non-split)
